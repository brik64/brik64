...
describe("Homepage Narrative — High Impact & Authority", () => {
  ...
  it("Agentic capability subtitle is technically optimized", () => {
    const content = read("src/components/vnext/home.tsx");
    expect(content).toContain("A finite 64-monomer bounded vocabulary, plus 64 extended monomers for controlled open interaction, mathematically optimized for rule-based reasoning and agentic code authoring.");
  });
});
...
describe("Industry Hero Backgrounds — Aerospace", () => {
  ...
  it("Aerospace hero keeps its metric cards opaque and the whole stack higher in the viewport", () => {
    ...
    expect(archetypes).toContain('mx-auto mt-14 grid max-w-5xl gap-4 sm:grid-cols-3');
    expect(archetypes).toContain('mx-auto mt-16 max-w-4xl border-t border-white/10 pt-10');
    expect(archetypes).toContain('className="border-white/14 bg-[#0f1a28] shadow-[0_18px_44px_rgba(0,0,0,0.28)]"');
